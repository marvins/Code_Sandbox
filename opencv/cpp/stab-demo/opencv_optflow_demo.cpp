...
int main(int argc, char **argv)
{
    ...
    std::vector<std::vector<std::tuple<cv::Point2f,cv::Point2f>>> line_list;
    ...
    while(true) {
        ...
        goodFeaturesToTrack(prev_grey, prev_corner, 2000, 0.01, 30);
        ...
        for(size_t i=0; i < status.size(); i++) {
            if(status[i]) {
                ...
                line_list.back().push_back(std::make_tuple(prev_corner[i],cur_corner[i]));
            }
        }
        ...
        if(T.data == NULL) {
            std::cout << "Using Old Transform" << std::endl;
            last_T.copyTo(T);
        }
    ...
    cv::VideoWriter vout;
    int width  = cap.get(CV_CAP_PROP_FRAME_WIDTH)*2+10;
    ...
    cv::Size outsize = cv::Size( width, height);
    vout.open( "output.avi",
    ...
    while(k < max_frames-1) { // don't process the very last frame, no valid transform
        ...
        // Write a line for each point
        for( auto vec : line_list[k] )
        {
            cv::line( cur, std::get<0>(vec), std::get<1>(vec), cv::Scalar(0,255,0), 2);
        }
